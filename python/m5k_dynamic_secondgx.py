...
target_stocks_ = pd.read_sql('select distinct stock_id from dfcf.m5k_result',con)
...
def update_macdx(df_macdx):
    ...
    if len(df_macdx) > 40:#上市40天的股票才考虑？
        ...
        if df_bottom_date is not None:
            df_bottom_date.to_sql(name='m5k_secondgx', con=con, if_exists='append', index=False)
