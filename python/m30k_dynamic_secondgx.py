...
target_stocks_ = pd.read_sql('select distinct stock_id from dfcf.m30k_result',con)
...
url_head_k = 'http://pdfm.eastmoney.com/em_ubg_pdti_fast/api/js?cb=jQuery183&TYPE=m30k&style=top&num=540&rtntype=1&id='
...
stock_info_init['url_m30k'] = url_list_k
...
def update_macdx(df_macdx):
    ...
    if len(df_macdx) > 40:#上市40天的股票才考虑？
        ...
        if df_bottom_date is not None:
            df_bottom_date.to_sql(name='m30k_secondgx', con=con, if_exists='append', index=False)
