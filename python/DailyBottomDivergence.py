...
#获取macd日线数据
sql_t_macd = 'select stock_id,date,low,diff,dea from dfcf.daily_macd'
...
#获取历史金叉日程数据
sql_t_xdate = 'select * from dfcf.daily_xdate'
...
if __name__ == '__main__':
    ...
    result.to_sql(name='daily_result', con=con, if_exists='replace', index=False) #写入动态调用库
    result.to_sql(name='daily_result_hist', con=con, if_exists='append', index=False) # 写入历史数据库
