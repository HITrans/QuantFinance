...
url_tail_sina = '&scale=5&ma=no&datalen=41'
...
def update_macdx(df_macdx):
    ...
    if len(df_macdx) > 40:#上市40天的股票才考虑？
        ...
        if df_bottom_date is not None:
            df_bottom_date.to_sql(name='m5k_xdate', con=con, if_exists='append', index=False)
...
if __name__ == '__main__':
    ...
    drop_exist_table = 'DROP TABLE IF EXISTS dfcf.dynamicm5;'
    cursor.execute(drop_exist_table)


    multicore()
    print('5分钟k线实时数据更新及macd计算完成')