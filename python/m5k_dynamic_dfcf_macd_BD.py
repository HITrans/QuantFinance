...
url_head_k = 'http://pdfm.eastmoney.com/em_ubg_pdti_fast/api/js?cb=jQuery183&TYPE=m5k&style=top&num=540&rtntype=1&id='
...
stock_info_init['url_m5k'] = url_list_k
    ...
    if len(df_temp_macd_i)>40:
        ...
        if new_bottom_x_i is not None:
            ...
            if cal_result is not None:
                cal_result.to_sql(name='m5k_result', con=con, if_exists='append', index=False)
                #print(cal_result)
...
def update_macdx(df_macdx):
    ...
    if len(df_macdx) > 40:#上市40天的股票才考虑？
        ...
        #df_temp_macd_t.to_sql(name='m5k_macd', con=con, if_exists='append', index=False)
        ...
        if df_bottom_date is not None:
            ...
            #df_bottom_date_hist.to_sql(name='df_bottom_m5k_hist', con=con, if_exists='append', index=False)
